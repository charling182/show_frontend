...
import { sendVerificationCode, putChangePassword } from '@/api';

interface RetrievePasswordFormProps {
    setStatus: Dispatch<React.SetStateAction<'login' | 'register' | 'retrievePassword'>>;
}

const RegisterForm = (props: RetrievePasswordFormProps) => {
    const { setStatus } = props;
    const [count, setCount] = useState(60); // 倒计时秒数
    const [timing, setTiming] = useState(false); // 是否在倒计时中
    const [formValues, setFormValues] = useState<types.user.changePasswordData | null>(null); // 保存表单数据的状态
    ...
    // 点击提交
    const handleFinish = async (values: types.user.changePasswordData) => {
        const { code } = await putChangePassword(values);
        if (code === 200) {
