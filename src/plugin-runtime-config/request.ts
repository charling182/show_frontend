...
const handleRequestInterceptors = (url: string, options: any) => {
    ...
    paramsArr.forEach((key) => {
        if (options.params[key] !== false && options.params[key] !== 0 && !options.params[key]) {
    ...
    let headers: any =
        options?.headers && Object.keys(options?.headers) ? { ...options?.headers } : {};
    if (appToken) {
        headers = {
            ...headers,
...
export const request: RequestConfig = {
        ...
        adaptor: (
            ...
            if (context.res instanceof Response) {
                ...
                Object.assign(errorInfo, {
                    ...
                    errorMessage:
                        errorInfo.msg || context.req.options.errorMessage || context.res.statusText,
                    showType:
            ...
            if (resp && resp.code && Object.prototype.hasOwnProperty.call(resp, 'visible')) {
                // 后端标准数据结构
                Object.assign(errorInfo, {
    ...
    responseInterceptors: [
        (response, options: RequestOptionsInit & RequestOptionsExtra) => {
            // 如果是401且不是在登录页，则跳转到登录页,401说明没有登录权限或者登录过期了
